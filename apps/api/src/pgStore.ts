...
// with an extra org_id tenant dimension on every query.
import { Pool } from "pg"
...
export class PgStore implements Store {
    ...
    async claimIdempotency(rec: {
        ...
        const { rows } = await this.pool.query(
            `insert into idempotency_keys (org_id, idem_key, fingerprint, method, path, created_at)
                  values ($1,$2,$3,$4,$5,$6)
             on conflict (org_id, idem_key) do nothing
               returning idem_key`,
            [rec.orgId, rec.idemKey, rec.fingerprint, rec.method, rec.path, rec.createdAt],
        )
        if (rows.length > 0) return { claimed: true }
        const existing = await this.getIdempotency(rec.orgId, rec.idemKey)
    ...
    private async getIdempotency(
        ...
        const { rows } = await this.pool.query(
            `select org_id, idem_key, fingerprint, method, path, status_code, response_body, created_at
               from idempotency_keys where org_id=$1 and idem_key=$2`,
            [orgId, idemKey],
        )
        if (!rows.length) return undefined
        const r = rows[0]
    ...
    async completeIdempotency(
        ...
        await this.pool.query(
            `update idempotency_keys set status_code=$3, response_body=$4 where org_id=$1 and idem_key=$2`,
            [orgId, idemKey, statusCode, JSON.stringify(responseBody ?? null)],
        )
    }
    async deleteIdempotency(orgId: string, idemKey: string): Promise<void> {
        await this.pool.query(
            `delete from idempotency_keys where org_id=$1 and idem_key=$2`,
            [orgId, idemKey],
        )
    }
}