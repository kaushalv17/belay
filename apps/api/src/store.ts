...
export interface Store {
    ...
    // --- Idempotency-Key replay (Phase 2 hardening) ---
    claimIdempotency(rec: {
        orgId: string
        idemKey: string
        fingerprint: string
        method: string
    ...
    deleteIdempotency(orgId: string, idemKey: string): Promise<void>
}
...
export class MemStore implements Store {
    ...
    private readonly idempotency = new Map<string, IdempotencyRecord>() // by `${orgId}\u0000${idemKey}`

    private akey(orgId: string, key: string): string {
        return `${orgId}\u0000${key}`
    }
    private ikey(orgId: string, idemKey: string): string {
    ...
    async completeIdempotency(
        ...
        const r = this.idempotency.get(this.ikey(orgId, idemKey))
        if (r) {
            r.statusCode = statusCode
            r.responseBody = responseBody ?? null
        }
    }
    async deleteIdempotency(orgId: string, idemKey: string): Promise<void> {
        this.idempotency.delete(this.ikey(orgId, idemKey))
    }
}